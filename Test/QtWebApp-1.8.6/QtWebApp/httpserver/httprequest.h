...
struct MultiMap {
    ...
    auto insert(Key_&& key, T_&& val) {
        return data_.emplace_back(std::forward<Key>(key), std::forward<T>(val));
    }
...
public:
    ...
    */
    ...
    MultiMap<QByteArray, QByteArray> getParameterMap() const;
...
private:
    ...
    MultiMap<QByteArray, QByteArray> parameters;
