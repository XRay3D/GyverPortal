...
String GPfileType(const String& uri) {
    ...
    if(div >= 0) {
        ...
        for(int i = 0; i < 5; i++) {
            int pos = inList(ext, FPSTR((const char*)pgm_read_dword(_gp_extension + i)));
            if(pos >= 0) {
                ext = listIdx(FPSTR(_gp_types), i);
                ext += '/';
                ext += listIdx(FPSTR((const char*)pgm_read_dword(_gp_value + i)), pos);
                return ext;
            }
        }
    }
...
GP_PGM_LIST(_gp_align, _gp_al0, _gp_al1, _gp_al2, _gp_al3, _gp_al0, _gp_al5, _gp_al6, _gp_al7);

PGM_P getAlignFlex(Align a) {
    return _gp_align[int(a)];
}
PGM_P getAlign(Align a) {
    return _gp_align[int(a) + 4];
}
...
// ================ STRING UTILS ==================
int inList(const String& s, const String& li) {
    int l = s.length();
