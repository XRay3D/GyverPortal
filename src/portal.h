...
extern ESP8266WebServer* __server__;
#else
#include <WebServer.h>
#endif
...
public:
    ...
    void start(const char* mdns = "", uint16_t port = 80) {
        ...
        server.on(
            "/GP_upload", HTTP_POST, [this]() {
            //server.send(200);
            server.send(200, "text/html", F("<script>setInterval(function(){if(history.length>0)window.history.back();else window.location.href='/';},500);</script>")); }, [this]() {
            if (!uplOn) return;
...
// ========================== OTA ==========================
    ...
    void enableOTA([[maybe_unused]] const String& login, [[maybe_unused]] const String& pass) {
#ifndef GP_NO_OTA
    ...
    void enableOTA() { enableOTA("", ""); }

    // ======================= АТТАЧИ =======================
    // подключить функцию-обработчик действия
    void attach(void (*handler)()) { _action = *handler; }
    void attach(void (*handler)(GyverPortal& p)) { _actionR = *handler; }
    ...
    void attachBuild(void (*handler)()) { _build = *handler; }
    void attachBuild(void (*handler)(GyverPortal& p)) { _buildR = *handler; }
...
*/
    ...
    // ArgParser virtual
    int args() override { return server.args(); }
    const String arg(const String& n) override { return server.arg(n); }
    bool hasArg(const String& n) override { return server.hasArg(n); }
    bool hasClick() override { return _clickF; }
    ...
    void checkList() {
        ...
        for (int i = 0; i < list.idx; i++) {
            if (list.skips[i] || form(list.forms[i])) {
                ...
                case Type::FLOAT:     *(float*)list.vals[i] = getFloat(      list.names[i]); return;
                case Type::COLOR:   *(GPcolor*)list.vals[i] = getColor(      list.names[i]); return;
                    // clang-format on
                }
            }
        }
